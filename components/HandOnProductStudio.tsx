...
import type { Language, ProductAnalysisResult } from '../types';
import { analyzeProductImage, generateHandOnProductStoryboard, generateHandOnProductKeyframe, retryOperation } from '../services/geminiService';
import { HAND_SKIN_TONES, HAND_GENDERS, LIGHTING_MODES } from '../constants';
import { 
...
interface HandOnProductStudioProps {
    language: Language;
...
const HandOnProductStudio: React.FC<HandOnProductStudioProps> = ({ language }) => {
    const [productImage, setProductImage] = useState<File | null>(null);
    const [productPreview, setProductPreview] = useState<string | null>(null);
    
    const [productName, setProductName] = useState('');
    const [productDescription, setProductDescription] = useState('');
    const [handSkinTone, setHandSkinTone] = useState('Light');
    const [handGender, setHandGender] = useState('Female');
    const [lighting, setLighting] = useState('Softbox Studio');
    ...
    const [scenes, setScenes] = useState<POVScene[]>([]);
    const [selectedScene, setSelectedScene] = useState<POVScene | null>(null);
    ...
    const handleProductUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsAnalyzing(true);
        try {
            const result: ProductAnalysisResult = await analyzeProductImage(file);
            setProductName(result.productName);
            setProductDescription(result.productDescription);
            setSuggestedThemes(result.backgroundThemes);
            if (result.backgroundThemes.length > 0) setSelectedBackground(result.backgroundThemes[0]);
    ...
    const handleGenerateStoryboard = async () => {
        ...
        setScenes([]);
        try {
            const result = await generateHandOnProductStoryboard(
                productName, productDescription, handGender, handSkinTone, 
                selectedBackground || 'Meja kayu minimalis', sceneCount,
    ...
    const handleGenerateKeyframe = async (scene: POVScene, index: number) => {
        if (!productImage) return;
        setScenes(prev => prev.map((s, i) => i === index ? { ...s, isGenerating: true } : s));
        try {
            await retryOperation(async () => {
                const base64 = await generateHandOnProductKeyframe(
                    productImage, productName, scene, handGender, handSkinTone,
                    selectedBackground || 'Meja kayu minimalis', lighting, usePro
                );
    ...
    const handleDownload = (scene: POVScene) => {
        if (!scene.imageBase64) return;
        const link = document.createElement('a');
        link.href = `data:image/png;base64,${scene.imageBase64}`;
        link.download = `pov_hand_scene_${scene.sceneNumber}.png`;
    ...
    return (
        <div className="flex flex-col lg:flex-row h-full relative overflow-y-auto lg:overflow-hidden bg-white">
            ...
            <div className="flex-shrink-0 lg:w-[420px] lg:flex-none order-1 lg:order-2 bg-white border-b-4 lg:border-b-0 lg:border-l-4 border-black flex flex-col lg:h-full lg:overflow-hidden z-10">
                <div className="p-4 lg:p-6 border-b-4 border-black bg-yellow-400">
                    ...
                    <p className="text-[10px] font-bold text-black/60 mt-1">Storyboard POV tangan untuk review produk</p>
                </div>
                    ...
                    <div className="space-y-6 lg:space-y-8">
                        ...
                        <div className="relative border-l-4 border-black pl-6 lg:pl-8 pb-4 ml-3">
                            <div className="absolute -left-[23px] lg:-left-[27px] top-0">
                                <span className="flex items-center justify-center w-10 h-10 lg:w-12 lg:h-12 bg-white border-4 border-black text-black font-black text-xs lg:text-sm shadow-neo-sm">1</span>
                            </div>
                            <label className="block text-sm font-black text-black uppercase mb-4">Product Upload</label>
                            <label className={`group flex flex-col justify-center px-4 py-6 border-4 border-black border-dashed relative transition-all overflow-hidden shadow-neo-sm hover:shadow-neo ${isAnalyzing ? 'cursor-wait bg-gray-50' : 'cursor-pointer bg-white hover:bg-yellow-50'}`}>
                                {isAnalyzing && (
                                    <div className="absolute inset-0 bg-white/90 flex flex-col items-center justify-center z-20">
                                        ...
                                        <p className="text-black mt-3 text-xs font-black uppercase tracking-widest animate-pulse">Analyzing...</p>
                                    </div>
                                )}
                                <div className="space-y-3 text-center relative z-10">
                                    {productPreview ? (
                                        <div className="relative inline-block border-4 border-black shadow-neo-sm">
                                            <img src={productPreview} alt="Preview" className="mx-auto h-32 w-auto object-contain" />
                                        </div>
                                    ) : (
                                        <div className="w-16 h-16 mx-auto bg-yellow-400 border-4 border-black flex items-center justify-center mb-2 shadow-neo-sm">
                                            <PhotoIcon className="h-8 w-8 text-black" />
                                        </div>
                                    )}
                                    {!productPreview && (
                                        <span className="font-black text-black uppercase underline decoration-4 decoration-yellow-400 text-xs">Click to upload</span>
                                    )}
                                </div>
                                <input type="file" className="sr-only" onChange={handleProductUpload} accept="image/*" />
                            </label>
                            <div className="mt-3 space-y-3">
                                <div>
                                    <label className={CYBER_LABEL}>Name</label>
                                    <input type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className={CYBER_INPUT} />
                                </div>
                                <div>
                                    <label className={CYBER_LABEL}>Description</label>
                                    <textarea rows={2} value={productDescription} onChange={(e) => setProductDescription(e.target.value)} className={CYBER_INPUT} />
                                </div>
                            </div>
                        </div>

                        {/* Step 2: Creative */}
                        <div className={`relative border-l-4 border-black pl-6 lg:pl-8 pb-4 ml-3 transition-all duration-500 ${analysisComplete ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                            <div className="absolute -left-[23px] lg:-left-[27px] top-0">
                            ...
                            <label className="block text-sm font-black text-black uppercase mb-4">Creative</label>
                            
                            <div className="bg-white p-4 border-4 border-black shadow-neo-sm space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                    <CyberDropdown label="Gender" value={handGender} options={HAND_GENDERS} onChange={setHandGender} />
                                    <CyberDropdown label="Skin" value={handSkinTone} options={HAND_SKIN_TONES} onChange={setHandSkinTone} />
                                </div>
                                <CyberDropdown label="Lighting" value={lighting} options={LIGHTING_MODES} onChange={setLighting} />
                                
                                <div>
                                    <label className={CYBER_LABEL}>Background</label>
                                    <button type="button" onClick={() => setIsBackgroundModalOpen(true)}
                                        className="w-full flex items-center justify-between bg-white border-4 border-black px-4 py-3 text-xs font-black uppercase hover:bg-yellow-50 transition-colors shadow-neo-sm">
                                        <span className={`truncate ${!selectedBackground ? 'text-gray-400' : 'text-black'}`}>
                                            {selectedBackground || "Select Theme..."}
                                        </span>
                                    </button>
                                </div>
                                    ...
                                    <div className="flex gap-2">
                                        {[3, 4, 5].map(n => (
                                            <button key={n} type="button" onClick={() => setSceneCount(n)}
                                                className={`flex-1 py-3 border-4 border-black text-xs font-black uppercase transition-all ${sceneCount === n ? 'bg-yellow-400 shadow-neo translate-x-[-2px] translate-y-[-2px]' : 'bg-white hover:bg-yellow-50'}`}>
                                                {n} Scene
                                            </button>
                                        ))}
                                    </div>
                        ...
                        <div className="space-y-3 pb-20 lg:pb-0">
                            <button onClick={handleGenerateStoryboard} disabled={isLoading || !productImage}
                            ...
                            {scenes.length > 0 && (
                                <button onClick={handleGenerateAll} disabled={scenes.some(s => s.isGenerating)}
                                    className="w-full flex justify-center items-center py-4 px-4 border-4 border-black shadow-neo text-sm font-black uppercase text-black bg-cyan-400 hover:bg-cyan-300 disabled:opacity-50 active:translate-x-[4px] active:translate-y-[4px] active:shadow-none transition-all">
            ...
            <div className="flex-none min-h-[50vh] lg:h-full lg:flex-1 order-2 lg:order-1 min-w-0 flex flex-col relative border-t-4 lg:border-t-0 lg:border-r-4 border-black bg-white">
                <div className="p-4 lg:p-6 border-b-4 border-black flex justify-between items-center bg-cyan-400">
                    <h2 className="text-sm lg:text-xl font-black text-black uppercase flex items-center tracking-tight">
                        <span className="w-8 h-8 lg:w-10 lg:h-10 border-4 border-black bg-white flex items-center justify-center mr-2 lg:mr-3 shadow-neo-sm">
                            <SparklesIcon className="w-5 h-5 lg:w-6 lg:h-6 text-black" />
                        </span>
                        POV Storyboard
                ...
                <div className="flex-1 lg:overflow-y-auto custom-scrollbar p-4 lg:p-6 bg-white">
                    {error && (
                        <div className="mb-6 p-4 border-4 border-black bg-pink-400 text-black text-sm font-black uppercase shadow-neo-sm">{error}</div>
                    )}

                    {isPlanning && (
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            <div className="relative w-20 h-20 lg:w-28 lg:h-28 mb-4 lg:mb-8 border-8 border-black bg-yellow-400 shadow-neo animate-bounce flex items-center justify-center">
                                <SparklesIcon className="w-10 h-10 text-black" />
                            </div>
                            <p className="text-xl lg:text-3xl font-black text-black uppercase tracking-tighter mb-2">Planning POV Storyboard...</p>
                            <p className="text-xs font-bold text-black/60 uppercase">AI sedang menyusun scene POV tangan untuk video review produkmu</p>
                        </div>
                    )}
                        ...
                        <div className="space-y-6">
                            {scenes.map((scene, index) => (
                                <div key={scene.id} className="border-4 border-black shadow-neo-sm bg-white overflow-hidden">
                                    <div className="flex items-center justify-between p-3 bg-yellow-100 border-b-4 border-black">
                                        <div className="flex items-center gap-3">
                                            <span className="w-10 h-10 bg-yellow-400 border-4 border-black flex items-center justify-center font-black text-sm shadow-neo-sm">{scene.sceneNumber}</span>
                                            <div>
                                        ...
                                        <button onClick={() => handleGenerateKeyframe(scene, index)} disabled={scene.isGenerating}
                                    ...
                                    <div className="flex flex-col lg:flex-row">
                                        ...
                                        <div className="flex-1 p-4 space-y-3">
                                            <div>
                                                <p className="text-[10px] font-black text-yellow-700 uppercase tracking-widest mb-1">✋ Visual Direction</p>
                    ...
                    {!isLoading && scenes.length === 0 && (
                        <div className="flex flex-col items-center justify-center h-full text-center py-20 lg:py-0">
                            <div className="w-20 h-20 lg:w-28 lg:h-28 bg-white border-4 border-black flex items-center justify-center mb-4 lg:mb-6 shadow-neo">
                                <PhotoIcon className="w-10 h-10 lg:w-14 lg:h-14 text-black/20" />
                            </div>
                            <p className="text-sm lg:text-xl font-black text-black uppercase mb-2">No Storyboard Yet</p>
                            <p className="text-xs font-bold text-black/40 uppercase">Upload produk dan generate storyboard POV</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <BackgroundGalleryModal 
                ...
                onConfirm={(themes) => { if(themes.length > 0) setSelectedBackground(themes[0]); }}
                suggestedThemes={suggestedThemes}
                selectedThemes={selectedBackground ? [selectedBackground] : []}
            />
                    ...
                    <div className="relative w-full lg:w-[80vw] h-[90vh] bg-white border-4 border-black shadow-neo-lg flex flex-col lg:flex-row overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="w-full lg:w-80 bg-white flex flex-col flex-shrink-0 h-1/3 lg:h-full">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                ...
                                <div className="p-4 border-4 border-black bg-yellow-50 space-y-3">
                                    <div>
...
export default HandOnProductStudio;